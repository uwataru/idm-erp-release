...
@Injectable()
export class OutsideMachiningService {
    ...
    GetById (pocNo:string): Observable<Item> {
        return this.http.get<Item>(this.url+'/forging-work-results/'+pocNo);
    }

    //======= 저장 =======//
    /** POST: 데이터 추가 */
    Create (data:Item): Observable<Item> {
        return this.http.post<Item>(this.url + '/outs-machining', data, httpOptions).pipe(
