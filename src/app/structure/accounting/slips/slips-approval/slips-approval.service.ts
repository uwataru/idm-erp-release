...
@Injectable()
export class SlipsApprovalService {
    ...
    GetAll (params): Observable<Item[]> {
        return this.http.get<Item[]>(this.url + '/approval-waiting', {params: params});
    }

    GetExcelFile (): Observable<Blob> {
        return this.http.get(this.url + '/approval-slips/exceldown', {responseType: 'blob'}).pipe(
            tap((data: Blob) => console.log(data)),
            catchError(this.handleError<Blob>('Create'))
        );
    }

    Approval (data:Item): Observable<Item> {
        return this.http.post<Item>(this.url + '/approval', data, httpOptions).pipe(
