...
export class SlipsComponent implements OnInit {
    ...
    constructor(
        ...
        private dataService: SlipsService,
    ...
    Save () {
        ...
        if(formData.brief_summary != ''){
            brief_summary_dr = formData.brief_summary;
        }else if(this.inputForm.value.dr_cr=="CR"&&formData.brief_summary==''){
    ...
    LoadSlipsToTemp (slipNo) {
        this.dataService.LoadSlipsToTemp(slipNo)
            .subscribe(
            data => {
                console.log("LoadSlipsToTemp in")
                if (data['result'] == "success") {
    ...
    //     }
    //
    //     this.searchForm.controls[event.target.id].patchValue(numbers.join('-'));
    // }
    ...
    onSelectAcctItems(acctCode):void {
        if (acctCode != '') {
            this.dataService.GetAcctItems(acctCode).subscribe(
                acctData =>
                {
                    console.log("MgmtItem3Name")
                    console.log(this.MgmtItem3Name)
                    console.log(acctData['data']['mgmt_item1_values'])
                    console.log(acctData['data']['mgmt_item3_no'])
                    if (acctData['data']['mgmt_item1_no'] != '') {
                        this.MgmtItem1No = acctData['data']['mgmt_item1_no'];
                        this.MgmtItem1Name = acctData['data']['mgmt_item1_name'];
                        if (acctData['data']['mgmt_item1_values'] != null) {
                    ...
                    if (acctData['data']['mgmt_item3_no'] != '') {
                        ...
                        console.log("event.returnValue in")
                        console.log(event.returnValue)

                        if (acctData['data']['mgmt_item3_values'] != null) {
                            this.MgmtItem3Values = acctData['data']['mgmt_item3_values'];
                            acctData['data']['mgmt_item3_no']='';
                        }
                    }
                    if (acctData['data']['mgmt_item4_no'] != '') {
                        this.MgmtItem4No = acctData['data']['mgmt_item4_no'];
    ...
    onSelectSlip(event): void {
        console.log("onSelectSlip")
        console.log(event.selected[0].slip_no)
